    ...
    OpenState = State() {
        ...
        ProjSpawn = function(self)
            if not self:IsDead() then
                ...
                local proj = self:CreateProjectile('/mods/BlackOpsFAF-ACUs/projectiles/SpysatSMDBait/SpysatSMDBait_proj.bp', loc[1], loc[2], loc[3], nil, nil, nil):SetCollision(false)
                Warp(proj, loc)
